...
export function HeroSection() {
  ...
  return (
    <AuroraBackground className="min-h-screen flex items-center justify-center py-20 overflow-hidden">
      <div className="container grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        >
          <div className="relative group">
            <motion.div
              className="absolute inset-0 rounded-full bg-gradient-to-r from-primary to-blue-500 opacity-70 blur-2xl group-hover:blur-3xl transition-all duration-500"
              animate={{
                scale: [1, 1.05, 1],
                opacity: [0.5, 0.7, 0.5],
              }}
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-8 order-1 md:order-2 mt-[70px] md:mt-0">
          <div className="space-y-4">
            ...
            >
              <motion.span
                key={displayText}
                className="text-3xl md:text-4xl font-bold text-primary font-sora min-h-[48px]"
              >
                {displayText}
                <motion.span
                  animate={{ opacity: [1, 0] }}
                  transition={{ duration: 0.5, repeat: Infinity }}
                  className="inline-block ml-1 w-[3px] h-[30px] bg-primary"
                />
              </motion.span>
          ...
          >
            <Button size="lg" className="group bg-primary hover:bg-primary/90 text-white rounded-full" asChild>
              <Link href="/projects">
                View Projects
                <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1" />
              </Link>
            </Button>
            <Button variant="outline" size="lg" className="group rounded-full dark:text-white dark:hover:text-white" asChild>
